...
const struct SpriteTemplate sAppIconSpriteTemplate =
{
    ...
    .callback = SpriteCallbackDummy, //SpriteCB_moveCursor
};
...
static void SpriteCB_moveCursor(struct Sprite *sprite)
{
    ...
    */
    if (sNewPokenavViewPtr->cursorIsMoving)
    {
        ...
        sprite->pos2.x = 50 + (sNewPokenavViewPtr->columnIndex * 70) - 120;
        sprite->pos2.y = (sNewPokenavViewPtr->rowIndex * 30) - 30;
        sNewPokenavViewPtr->cursorPosX = sprite->pos2.x;
        sNewPokenavViewPtr->cursorPosY = sprite->pos2.y;
...
static void CreateSelectionCursor(u8 scenario)
{
    ...
    {
        gSprites[spriteId].pos2.x = 50 + (sNewPokenavViewPtr->columnIndex * 70) - 120;
        gSprites[spriteId].pos2.y = (sNewPokenavViewPtr->rowIndex * 30) - 30;
    }
...
static void Task_LoadIcons(u8 taskId)
{
    if (gTasks[taskId].tFrameCounter < 3)
    {
        u8 spriteId;
        struct CompressedSpriteSheet spriteSheet;
        ...
        spriteId = CreateSprite(&sAppIconSpriteTemplate, 50 + (gTasks[taskId].tFrameCounter * 70), 69, 0);
