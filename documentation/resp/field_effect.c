...
static void Task_UseFly(u8 taskId)
{
    ...
    if (!task->data[0])
    {
        ...
        gFieldEffectArguments[0] = GetCursorSelectionMonId(); // Rescata el índice del pokémon que usó fly
...
#define tPelipperTaxiFlag   data[3] //re-used
...
static void FlyOutFieldEffect_ShowMon(struct Task *task)
{
    ...
    {
        task->tState++;
        gFieldEffectArguments[0] = task->tMonId; // Indice del pokémon en party que usa el movimiento
        if (VarGet(VAR_UNUSED_0x40E5) == 1)
        {
            // task->tPelipperTaxiFlag = 1; // Borrar
            FieldEffectStart(FLDEFF_USE_PELIPPER_TAXI); // Si cambiamos esto deja de mostrar el mon
...
static void SpriteCB_FlyBirdLeaveBall(struct Sprite *sprite)
{
    if (sprite->sAnimCompleted == FALSE)
    {
        if (sprite->data[0] == 0)
        {
            ...
            sprite->pos1.x = 0x76;          // (0x76 , -0x30) = Mid Screen
            sprite->pos1.y = -0x30;
            sprite->data[0]++;
            sprite->data[1] = 0x40;
            sprite->data[2] = 0x100;
        ...
        /**
           * * Quitando las dos lineas posteriores, se
           * * remueve el primer loop de la animación

        */
        if (VarGet(VAR_UNUSED_0x40E5) == 0)
        {
